...
> ⚠️ **fork 过时点注记（2026-09-05 调研）**：① 本仓库 Web 服务默认端口为 **1122**（可设置项 webPort 调整），下文示例中的 1234 均为旧默认值；
> ② 文中要求的 `io.legado.READ_WRITE` 权限在本 fork 不存在，`readerProvider` 实际导出且无权限保护。详见 [项目地图与状态报告](docs/superpowers/reports/2026-09-05-项目地图-状态报告.md) §5/§6。
## 对于Web的配置
您需要先在设置中启用"Web 服务"。  
## 使用
### Web
